...
export default class VotacionProfesor extends Component {
    ...
    async filterCharlas() {
        ...
        const charlasWithVotes = await Promise.all(charlasByEstado.map(async (charla) => {
            const res = await services.getVotosCharla(charla.idCharla);
            const votos = res && !isNaN(res.votos) ? res.votos : 0;
            return { ...charla, votos: votos };
        }));
    ...
    sortCharlasByVotes = () => {
        const sortedCharlas = [...this.state.charlas].sort((a, b) => a.votos > b.votos);
        this.setState({ charlas: sortedCharlas });
    }
    ...
    getVotosCharla = (id) => {
        console.log("getvotos id charla", id)
        services.getVotosCharla(id).then(res => {
            this.setState({ votosCharla: res.votos })
            console.log(this.state.votosCharla);
        }).catch(err => {
            console.log(err);
        })
    }
    ...
    handleCardClick = (charla) => {
        ...
        console.log("charla seleccionada", charla);
        this.getVotosCharla(charla.idCharla);
    }
    ...
    updateEstadoCharla = () => {
        console.log("seleccionadaCharla ", this.state.seleccionadaCharla)
        var estado = this.state.seleccionadaCharla.idEstadoCharla === 1 ? 2 : 1;
        var estadoEscrito = estado === 1 ? "propuesta" : "aceptada";
        var numVotos = this.state.votosCharla ?? 0;
        Swal.fire({
            title: "¿Cambiar a " + estadoEscrito + "?",
            text: "Esta charla tuvo " + numVotos + " votos.",
            icon: "ingo",
